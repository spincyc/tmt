...
| Test (`tools/<id>.test`) | Executable that exits 0; the price of `stable` |
| Candidate | A recorded "re-derived X again" event, stored by aiq |
| Origin stamp | Provenance on a vendored copy: source repo, commit, sha256 |
...
|---|---|---|
| `draft` | `tmt new <id>` + paste the derived logic | registry validity, entry↔file parity, syntax lint, executable bit, `--help` smoke, `requires` resolve, no cycles |
| `stable` | add `tools/<id>.test`, flip `stage` by hand | draft gates + test exits 0 + no draft dependencies + no hardcoded absolute paths |

Drafts may live indefinitely; the only penalty is exclusion from stable
composition. Promotion is a hand edit of `tmt.json` — there is no promote
command — and `tmt check` is one line in the repo's existing verify target,
...
and declares `<dep>` in its `requires` list. `tmt check` keeps the graph
...
5. Edge cases are fixed in place (prefer editing over near-duplicates); a
   test is added; `stage` flips to `stable`. Git history is the audit trail.
...
| 2 | `tmt-lib` — an ordinary tmt-enabled repo containing only tools | `tmt adopt <id> --to ~/git/tmt-lib`; a human commits the promotion |
