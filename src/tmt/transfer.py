...
``origin`` with the source repo path, commit, and content sha256. Vendoring
and re-adoption are deliberate overwrites; divergence afterwards is allowed.
"""
...
def vendor(root: Path, source: Path, tool_id: str) -> dict[str, Any]:
    ...
    origin = {
        ...
        "sha256": checks.sha256_file(dest_tool),
    }
    stamped = dict(entry)
